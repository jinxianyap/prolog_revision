...
#constant(hid_type, pbl_r4_1_furniture_X_var_vals_var_val_r4_var_x_X_end).

#modeh(head_extension(rev1, const(head_extension_type), pbl(r4, 1, living(var(ground)), var_vals(var_val(r4, var_x, var(ground)), end)))).
#constant(head_extension_type, pbl_r4_1_living_X_var_vals_var_val_r4_var_x_X_end).

#bias(":- head(head_delete(_, _)), body(_).").
#bias(":- head(head_extension(_, _, _)), body(_).").
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
prove(X) : possible_head(R, V, X) :- extension(R, V).
pbl(r4, 1, furniture(X), var_vals(var_val(r4, var_x, X), end)) :- prove(pbl(r4, 1, furniture(X), var_vals(var_val(r4, var_x, X), end))).
pbl(r4, 1, living(X), var_vals(var_val(r4, var_x, X), end)) :- prove(pbl(r4, 1, living(X), var_vals(var_val(r4, var_x, X), end))).

%Try and extension rules

possible_head(rev1, v(X), pbl(r4, 1, furniture(X), var_vals(var_val(r4, var_x, X), end))) :-
	not head_delete(rev1, pbl_r4_1_furniture_X_var_vals_var_val_r4_var_x_X_end),
	try(rev1, ground_X, ground(X)),
	extension(rev1, v(X)).

possible_head(rev1, v(X), pbl(r4, 1, living(X), var_vals(var_val(r4, var_x, X), end))) :-
	head_extension(rev1, pbl_r4_1_living_X_var_vals_var_val_r4_var_x_X_end, pbl(r4, 1, living(X), var_vals(var_val(r4, var_x, X), end))),
	try(rev1, ground_X, ground(X)),
	extension(rev1, v(X)).
...
possible_head(rev1, v(X), null) :-
	not empty_head(rev1),
	not possible_head(rev1, v(X), pbl(r4, 1, furniture(X), var_vals(var_val(r4, var_x, X), end))),
	not possible_head(rev1, v(X), pbl(r4, 1, living(X), var_vals(var_val(r4, var_x, X), end))),
	ground(X).

empty_head(rev1) :-
	head_delete(rev1, pbl_r4_1_furniture_X_var_vals_var_val_r4_var_x_X_end),
	not head_extension(rev1, pbl_r4_1_living_X_var_vals_var_val_r4_var_x_X_end, pbl(r4, 1, living(X), var_vals(var_val(r4, var_x, X), end))),
	ground(X).
